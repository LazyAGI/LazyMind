...
class RuntimeSupervisor:
    ...
    def tick(self, *, cursor: int = 0, now: float, tick_id: str, run_ids: tuple[str, ...] = ()) -> TickResult:
        ...
        return TickResult(started, cursor, tuple(recovered), dispatched, tuple(dispatch_results), tuple(notices), partial_sync)
    ...
    def _dispatch_candidate_run_ids(self, scope: tuple[str, ...]) -> tuple[str, ...]:
        ...
        projections = self.store.list_runs(RunQuery(statuses=("running",), has_open_attempts=True, limit=self.policy.max_runs_per_tick))
