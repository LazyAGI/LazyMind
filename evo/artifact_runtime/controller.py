...
ATTEMPT_EVENTS: dict[str, AttemptStatus] = {f"attempt.{k}": k for k in ("completed", "failed", "stale", "cancelled", "cancel_requested")}
...
class RunState:
    run_id: str; status: RunStatus = "pending"; active_plan_version: int | None = None; epoch: int = 0
    def __post_init__(self) -> None: validate_nonempty(self.run_id, "run_id")


@dataclass(frozen=True)
class PlanInstance:
    run_id: str; plan_id: str; plan_version: int; epoch: int; graph_revision: int
    target_artifacts: tuple[ArtifactKey, ...]; reason: str; plan: ExecutionPlan


@dataclass(frozen=True)
class Attempt:
    attempt_id: str; run_id: str; plan_version: int; epoch: int; op_id: str
    resolved_input_refs: dict[ArtifactKey, ArtifactRef]; output_artifact_keys: tuple[ArtifactKey, ...]; depends_on: tuple[str, ...]
    status: AttemptStatus = "pending"; attempt_number: int = 1; claim_id: str = ""
    output_refs: dict[ArtifactKey, ArtifactRef] = field(default_factory=dict); reason: str = ""
    worker_id: str = ""; lease_expires_at: float = 0.0; claim_generation: int = 0; lease_recovery_count: int = 0
...
@dataclass(frozen=True)
class AttemptClaim:
    claim_id: str; attempt_id: str; run_id: str; plan_version: int; epoch: int; op_id: str; plan_op: PlanOp
    resolved_input_refs: dict[ArtifactKey, ArtifactRef]; output_artifact_keys: tuple[ArtifactKey, ...]; cancel_requested: bool = False
    worker_id: str = ""; lease_expires_at: float = 0.0; claim_generation: int = 0


@dataclass(frozen=True)
class AttemptExecutionResult:
    ok: bool; outputs: dict[str, Any] = field(default_factory=dict); error_type: str = ""; error_message: str = ""


@dataclass(frozen=True)
class CommitResult:
    status: CommitStatus; output_refs: dict[ArtifactKey, ArtifactRef] = field(default_factory=dict); reason: str = ""
...
@dataclass(frozen=True)
class AttemptResult:
    attempt_id: str; status: AttemptStatus; commit_status: CommitStatus | None = None; reason: str = ""


@dataclass(frozen=True)
class ClaimHeartbeatResult:
    status: ClaimHeartbeatStatus; attempt_id: str; claim_id: str


@dataclass(frozen=True)
class ClaimInspectionResult:
    status: ClaimInspectionStatus; attempt_id: str; claim_id: str; cancel_requested: bool = False
    attempt_status: AttemptStatus | None = None; reason: str = ""
...
class ControllerEvent:
    event_type: str; run_id: str; payload: dict[str, Any] = field(default_factory=dict); seq: int = 0
    def __post_init__(self) -> None:
        validate_nonempty(self.event_type, "event_type"); validate_nonempty(self.run_id, "run_id")
...
class InMemoryEventLog:
    def __init__(self) -> None: self._events: list[ControllerEvent] = []

    def append(self, event: ControllerEvent) -> int:
        self._events.append(replace(event, seq=len(self._events) + 1)); return self._events[-1].seq
...
class ControllerState:
    run: RunState; run_exists: bool; plans: dict[int, PlanInstance]; attempts: dict[str, Attempt]; command_results: dict[str, Any]
...
class RunController:
    ...
    def create_run(self, run_id: str) -> RunState:
        with self._lock:
            ...
            if not state.run_exists:
                self._append(run_id, "run.created"); state = self.state(run_id)
            return state.run
    ...
    def submit_plan(self, run_id: str, plan: ExecutionPlan, *, targets: set[ArtifactKey],
        ...
        with self._lock:
            ...
            if seen is not _MISSING: return state.plans[int(seen["plan_version"])]
            if not state.run_exists:
                self._append(run_id, "run.created"); state = self.state(run_id)
            if state.run.status in RUN_PLAN_BLOCKED: raise ValueError(f"cannot submit plan to {state.run.status} run")
            if state.run.active_plan_version is not None:
                ...
                for attempt in state.attempts.values():
                    if attempt.plan_version == state.run.active_plan_version and attempt.status in ATTEMPT_OPEN:
                        self._append(run_id, "attempt.stale", {"attempt_id": attempt.attempt_id, "reason": "plan_superseded"})
    ...
    def claim_ready(self, run_id: str, *, limit: int = 1, worker_id: str = "", lease_expires_at: float = 0) -> list[AttemptClaim]:
        if limit < 1:
            raise ValueError("limit must be >= 1")
        with self._lock:
            _validate_claim_request(worker_id, lease_expires_at)
            state = self.state(run_id); claims: list[AttemptClaim] = []
            for attempt in self._ready_attempts(state)[:limit]:
                ...
                claim = AttemptClaim(claim_id, attempt.attempt_id, attempt.run_id,
                                     ...
                                     self._resolve_inputs_at_claim(state, attempt, plan_op), attempt.output_artifact_keys,
                                     worker_id=worker_id, lease_expires_at=float(lease_expires_at),
                                     claim_generation=claim_generation)
                self._append(run_id, "attempt.claimed", {"attempt_id": attempt.attempt_id, "claim_id": claim.claim_id,
    ...
    def complete_attempt(self, claim: AttemptClaim, result: AttemptExecutionResult) -> AttemptResult:
        with self._lock:
            ...
            if terminal := _terminal_result(attempt): return terminal
            ...
            if self.committer is None: raise RuntimeError("RunController requires a committer to complete attempts")
            commit = self.committer.commit_attempt(attempt, self._plan_op_for_attempt(state, attempt), result)
            if commit.status == "committed":
                return self._finish_attempt(attempt, "completed", commit_status="committed", output_refs=commit.output_refs)
            if commit.status == "stale":
                return self._finish_attempt(attempt, "stale", commit_status="stale", reason=commit.reason)
            out = self._record_failure(attempt, AttemptExecutionResult(False, error_message=commit.reason or commit.status), commit.status)
            self._maybe_finish_run(attempt.run_id); return out
    ...
    def fail_attempt(self, claim: AttemptClaim, error: dict[str, Any]) -> AttemptResult:
        with self._lock:
            ...
            out = self._record_failure(attempt, result); self._maybe_finish_run(attempt.run_id); return out
    ...
    def inspect_claim(self, claim: AttemptClaim) -> ClaimInspectionResult:
        with self._lock:
            ...
            if attempt.status in ATTEMPT_TERMINAL:
                return ClaimInspectionResult("terminal", claim.attempt_id, claim.claim_id, attempt_status=attempt.status, reason=attempt.reason)
            return ClaimInspectionResult("current", claim.attempt_id, claim.claim_id, attempt.status == "cancel_requested", attempt.status)
    ...
    def request_cancel(self, run_id: str, *, command_id: str) -> RunState:
        with self._lock:
            ...
            if seen is not _MISSING or state.run.status in RUN_PLAN_BLOCKED:
                if seen is _MISSING: self._record_command(run_id, command_id, "cancel", {"status": state.run.status})
                return state.run
            active = state.run.active_plan_version
            for attempt in state.attempts.values():
                if attempt.plan_version == active and attempt.status in {"pending", "claimed"}:
                    self._append(run_id, f"attempt.{'cancelled' if attempt.status == 'pending' else 'cancel_requested'}",
                                 {"attempt_id": attempt.attempt_id})
            self._append(run_id, "run.cancel_requested", {"command_id": command_id}); self._maybe_finish_run(run_id)
    ...
    def retry_failed(self, run_id: str, *, command_id: str) -> list[Attempt]:
        with self._lock:
            ...
            if seen is not _MISSING: return [state.attempts[item] for item in seen.get("attempt_ids", []) if item in state.attempts]
            created = [self._schedule_retry(run_id, state.active_plan, attempt) for attempt in state.attempts_by_status.get("failed", ())
                       if state.active_plan is not None and self._retryable_latest(state, attempt)]
            self._record_command(run_id, command_id, "retry_failed", {"attempt_ids": [attempt.attempt_id for attempt in created]})
            if created and state.run.status == "failed": self._append(run_id, "run.resumed")
            return created

    def recover_expired_claims(self, run_id: str, *, now: float, max_recoveries: int, command_id: str) -> LeaseRecoveryResult:
        if max_recoveries < 0:
    ...
    def _seen_command(self, run_id: str, command_id: str, *, kind: str, require_run: bool = True) -> tuple[Any, ControllerState]:
        validate_nonempty(command_id, "command_id"); state = self.state(run_id)
        if require_run and not state.run_exists: raise ValueError(f"unknown run: {run_id}")
        seen = state.command_results.get(command_id, _MISSING)
        if seen is not _MISSING and seen.get("kind") != kind: raise ValueError(f"command_id reused for {seen.get('kind')}: {command_id}")
        return seen, state

    def _status_command(self, run_id: str, command_id: str, kind: str, expected: RunStatus, event_type: str) -> RunState:
        seen, state = self._seen_command(run_id, command_id, kind=kind)
        if seen is not _MISSING: return replace(state.run, status=seen.get("status", state.run.status))
        self._append(run_id, event_type, {"command_id": command_id}) if state.run.status == expected else self._record_command(run_id, command_id, kind, {"status": state.run.status})
        return self.state(run_id).run
    ...
    def _finish_attempt(self, attempt: Attempt, status: AttemptStatus, *, commit_status: CommitStatus | None = None,
        ...
        if output_refs is not None: payload["output_refs"] = output_refs
        self._append(attempt.run_id, f"attempt.{status}", payload); self._maybe_finish_run(attempt.run_id)
        return AttemptResult(attempt.attempt_id, status, commit_status, reason)

    def _record_failure(self, attempt: Attempt, result: AttemptExecutionResult, commit_status: CommitStatus | None = None) -> AttemptResult:
    ...
    def _schedule_retry(self, run_id: str, plan: PlanInstance, attempt: Attempt) -> Attempt:
        retry = self._new_attempt(run_id, plan, self._plan_op_by_id(plan.plan, attempt.op_id), attempt.attempt_number + 1)
    ...
    def _retryable_latest(self, state: ControllerState, attempt: Attempt) -> bool:
        return attempt.plan_version == state.run.active_plan_version and attempt.attempt_number < self.retry_policy.max_attempts \
    ...
    def _ready_attempts(self, state: ControllerState) -> list[Attempt]:
        active = state.run.active_plan_version
        if state.run.status != "running" or active is None: return []
        ...
        return [attempt for attempt in state.attempts_by_status.get("pending", ())
                if attempt.plan_version == active
                and not any((dep := state.latest_attempt_by_op.get((active, op_id))) is None or dep.status != "completed" for op_id in attempt.depends_on)
                and not any(artifact_id in claimed_outputs for artifact_id in attempt.output_artifact_keys)]

    def _resolve_inputs_at_claim(self, state: ControllerState, attempt: Attempt, plan_op: PlanOp) -> dict[ArtifactKey, ArtifactRef]:
    ...
    def _plan_op_by_id(plan: ExecutionPlan, op_id: str) -> PlanOp:
        try: return next(plan_op for plan_op in _plan_ops(plan) if plan_op.op_id == op_id)
        except StopIteration: raise ValueError(f"plan op not found: {op_id}") from None

    def _maybe_finish_run(self, run_id: str) -> None:
        state = self.state(run_id)
        if state.run.status not in {"running", "cancel_requested"} or state.run.active_plan_version is None: return
        statuses = {attempt.status for (version, _), attempt in state.latest_attempt_by_op.items() if version == state.run.active_plan_version}
        if state.run.status == "cancel_requested" and statuses and not statuses.intersection(ATTEMPT_OPEN): self._append(run_id, "run.cancelled")
        elif statuses.intersection({"failed", "stale"}): self._append(run_id, "run.failed")
        elif statuses == {"completed"}: self._append(run_id, "run.completed")

    def _new_attempt(self, run_id: str, plan: PlanInstance, plan_op: PlanOp, attempt_number: int) -> Attempt:
        return Attempt(f"plan{plan.plan_version}:{plan_op.op_id}:{attempt_number}", run_id, plan.plan_version, plan.epoch,
                       plan_op.op_id, {}, plan_op.output_keys,
                       plan_op.depends_on, attempt_number=attempt_number)
    ...
    def _attempt_for_claim(self, claim: AttemptClaim) -> Attempt:
        attempt = self.state(claim.run_id).attempts.get(claim.attempt_id)
        if attempt is None or attempt.claim_id != claim.claim_id: raise ValueError(f"unknown claim_id: {claim.claim_id}")
        return attempt
...
def _replay(run_id: str, events: Iterable[ControllerEvent]) -> ControllerState:
    data: dict[str, Any] = {"run": RunState(run_id), "run_exists": False, "plans": {}, "attempts": {}, "command_results": {}}
    for event in sorted(events, key=lambda item: item.seq):
        payload = event.payload
        if event.event_type == "run.created":
            data["run_exists"] = True; data["run"] = replace(data["run"], status="pending")
        elif event.event_type in RUN_EVENTS:
            _record_command(data, event, {"kind": RUN_COMMAND_KINDS.get(event.event_type, event.event_type), "status": RUN_EVENTS[event.event_type]})
            data["run"] = replace(data["run"], status=RUN_EVENTS[event.event_type])
        elif event.event_type == "plan.submitted":
            plan = payload["plan"]; data["plans"][plan.plan_version] = plan
            ...
            attempt = payload["attempt"]; data["attempts"][attempt.attempt_id] = attempt
            ...
            _update_attempt_if_claim_matches(data["attempts"], payload["attempt_id"], payload.get("claim_id"),
                                             status="pending", claim_id="", worker_id="", lease_expires_at=0.0,
                                             resolved_input_refs={}, reason=str(payload.get("reason") or ""),
                                             lease_recovery_count=_next_recovery_count(data["attempts"], payload["attempt_id"]))
            ...
            if "claim_id" in payload:
                _update_attempt_if_claim_matches(data["attempts"], payload["attempt_id"], payload.get("claim_id"), **updates)
            else:
...
def _record_command(data: dict[str, Any], event: ControllerEvent, result: Any) -> None:
    if command_id := str(event.payload.get("command_id") or ""): data["command_results"][command_id] = result


def _build_state(data: dict[str, Any]) -> ControllerState:
    latest: dict[tuple[int, str], Attempt] = {}; producer: dict[tuple[int, ArtifactKey], Attempt] = {}; by_status: dict[AttemptStatus, list[Attempt]] = {}
    for attempt in data["attempts"].values():
        key = (attempt.plan_version, attempt.op_id); by_status.setdefault(attempt.status, []).append(attempt)
        if key not in latest or attempt.attempt_number > latest[key].attempt_number: latest[key] = attempt
        if attempt.status == "completed": producer.update({(attempt.plan_version, artifact_id): attempt for artifact_id in attempt.output_refs})
...
def _update_attempt(attempts: dict[str, Attempt], attempt_id: str, **changes: Any) -> None:
    if attempt_id in attempts: attempts[attempt_id] = replace(attempts[attempt_id], **changes)


def _update_attempt_if_claim_matches(attempts: dict[str, Attempt], attempt_id: str, expected_claim_id: Any, **changes: Any) -> None:
...
def _terminal_result(attempt: Attempt) -> AttemptResult | None:
    return AttemptResult(attempt.attempt_id, attempt.status, reason=attempt.reason) if attempt.status in ATTEMPT_TERMINAL else None
...
def _validate_claim_request(worker_id: str, lease_expires_at: float) -> None:
    if (not worker_id and lease_expires_at != 0) or (worker_id and lease_expires_at <= 0):
        raise ValueError("claim must be non-leased (worker_id='', lease_expires_at=0) or leased (worker_id set, lease_expires_at > 0)")
