...
class RunProjection:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "attempts", tuple(sorted(self.attempts, key=lambda item: (item.plan_version, item.op_id, item.attempt_id))))
        object.__setattr__(
            self,
            "artifact_producers",
...
class InMemoryRunProjectionStore:
        ...
        for projection in projections:
            ...
            for attempt in projection.attempts:
                ...
                if attempt.lease_expires_at <= 0 or attempt.lease_expires_at > now:
                    continue
                candidates.append(ExpiredLeaseCandidate(attempt.run_id, attempt.attempt_id, attempt.lease_expires_at, attempt.worker_id))
    ...
    def list_artifact_producers(
        ...
        *,
        ...
        with self._lock:
            producers = tuple(producer for projection in self._runs.values() for producer in projection.artifact_producers)
        if key is not None:
...
class ProjectionBuilder:
    def from_state(self, state: ControllerState, *, updated_seq: int) -> RunProjection:
        ...
        open_ids = tuple(sorted(attempt.attempt_id for attempt in active_attempts if attempt.status in _OPEN_ATTEMPT_STATUSES))
        target_artifacts = state.active_plan.target_artifacts if state.active_plan is not None else ()
        producers = tuple(
            ArtifactProducerProjection(key, ref, attempt.run_id, attempt.attempt_id, attempt.op_id, attempt.plan_version)
...
class ProjectionSynchronizer:
    ...
    def sync_run(self, run_id: str) -> RunProjection:
        ...
        projection = self.builder.from_state(self.controller.state(run_id), updated_seq=max(event.seq for event in events))
...
def _summary_reason(attempts) -> str:
    for status in _SUMMARY_REASON_PRIORITY:
        for attempt in sorted((item for item in attempts if item.status == status and item.reason), key=lambda item: item.attempt_id):
