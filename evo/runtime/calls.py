...
class InMemoryCallRecorder:
    ...
    def record(self, adapter_type: str, request: Any, response: Any = None, *, phase: str = '', item_ref: str = '',
        ...
        if idempotency_key:
            ...
            if existing is not None:
                return self._reuse(existing, phase=phase, item_ref=item_ref)
        record = CallRecord(
    ...
    def succeeded(self, idempotency_key: str, *, idempotency_scope: str = 'operation') -> CallRecord | None:
        ...
        for record in records:
            if record.idempotency_key == idempotency_key and record.idempotency_scope == idempotency_scope and record.status == 'succeeded':
                return record
        return None
    ...
    def _reuse(self, existing: CallRecord, *, phase: str, item_ref: str) -> CallRecord:
        ...
        record = CallRecord(
            ...
            idempotency_scope=existing.idempotency_scope,
