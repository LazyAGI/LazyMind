...
class AdapterCall:
    ...
    def run(self, ctx: OperationContext, request: dict, *, phase: str = '', item_ref: str = '', idempotency_key: str = '',
        ...
        if existing is not None:
            ctx.check_interrupt()
            record = ctx.call_recorder.record(
                self.adapter_type,
                request,
                existing.response,
                phase=phase,
        ...
        try:
            ...
            if ctx.interrupt_requested():
                raise OperationInterrupted(ctx.operation_run_id) from exc
            record = ctx.call_recorder.record(
                self.adapter_type,
                request,
                None,
...
def _idempotency_key(adapter_type: str, request: dict) -> str:
    payload = json.dumps({'adapter_type': adapter_type, 'request': request}, ensure_ascii=False, sort_keys=True, default=str)
