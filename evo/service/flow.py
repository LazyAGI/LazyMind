...
class EvoFlowService:
    def __init__(self, *, run_root: Path | str, run_id: str = 'run_1', dataset_id: str, target_chat_url: str,
                 candidate_chat_url: str = '', router_admin_url: str = '', case_count: int = 20,
                 max_workers: int = 4, model_config: dict[str, Any] | None = None):
        self._setup(
            run_root=run_root,
            run_id=run_id,
    ...
    @classmethod
    def resume(cls, *, run_root: Path | str, run_id: str = 'run_1', dataset_id: str, target_chat_url: str,
               candidate_chat_url: str = '', router_admin_url: str = '', case_count: int = 20,
               max_workers: int = 4, model_config: dict[str, Any] | None = None) -> 'EvoFlowService':
    ...
    def _setup(self, *, run_root: Path | str, run_id: str, dataset_id: str, target_chat_url: str,
               candidate_chat_url: str, router_admin_url: str, case_count: int, max_workers: int,
               model_config: dict[str, Any] | None, recover: bool = False) -> None:
    ...
    def run_full_flow(self, *, include_repair_loop: bool = True, include_abtest: bool = True,
                      ...
                      repair_plan_params: dict[str, Any] | None = None, after_stage: AfterStage | None = None,
                      ) -> dict[str, list[OperationResult]]:
        next_steps = {
            'dataset': ('eval', 'eval.run'),
            'eval': ('analysis', 'analysis.run'),
            'analysis': ('repair', 'repair.run'),
            'repair': ('abtest', 'abtest.compare'),
        }
            ...
            try:
                ...
                if accepted and not _has_latest(self.artifacts, 'candidate_algorithm_cutover'):
                    notify(
                        ...
                        checkpoint_kind='manual_cutover',
    ...
    def create_candidate_cutover_run(self) -> OperationRunRef | None:
        ...
        if (comparison.get('decision') or {}).get('status') != 'accept':
            ...
            return None
        if not self.router_admin_url:
            raise RuntimeError('candidate cutover requires router_admin_url')
        workspace_ref = self.artifacts.latest_ref('candidate_workspace')
    ...
    def execute_candidate_cutover(self, message_id: str = 'msg_manual_cutover') -> tuple[OperationRunRef | None, list[OperationResult], bool]:
        if _has_latest(self.artifacts, 'candidate_algorithm_cutover'):
            return None, [], True
        cutover_ref = self.graph.active_run_for('abtest.candidate_cutover')
        if not cutover_ref:
            cutover_ref = self.create_candidate_cutover_run()
        if not cutover_ref:
            return None, [], False
        results = self._dispatch_stage(
            'candidate_cutover', f'{message_id}_candidate_cutover', ['candidate_algorithm_cutover']
        )
    ...
    def _flow_progress(self, stage: str, status: str, message: str,
    ...
    def _registry(self) -> CapabilityRegistry:
        ...
        return CapabilityRegistry(step_capabilities(
            ...
            router_admin_url=self.router_admin_url,
    ...
    def _create_eval_report_runs(self, prefix: str, dataset_ref: ArtifactRef, chat_url: str, report_id: str, *,
                                 depends_on: list[OperationRunRef] | None = None,
                                 candidate_service_ref: str = '', flow_tag: str = 'eval',
    ...
    def _apply_control(
        self, result, message_id: str, *, dispatch: bool, max_dispatch: int | None,
    ) -> tuple[list[OperationResult], bool]:
        if not result.intents:
            return [], False
        intent = result.intents[0]
        if intent.action not in {
            'retry_operation', 'cancel_operation', 'cancel_running_operation', 'resume_checkpointed',
        }:
            return [], False
        for proposal in result.proposals:
        ...
        if intent.action == 'resume_checkpointed':
            refs = self.graph.run_refs({'checkpointed'})
            return (self.resume_checkpointed() if refs else []), True
        if intent.action == 'retry_operation':
            ...
            return self._dispatch_refs(refs, message_id), True
    ...
    def _dispatch_refs(self, refs: list[OperationRunRef], message_id: str) -> list[OperationResult]:
        old_limit, old_workers = self.runtime.max_dispatch, self.runtime.max_workers
        self.runtime.max_dispatch, self.runtime.max_workers = 1, 1
        try:
            outputs = []
...
def _dataset_difficulty(index: int, total: int) -> str:
    if total <= 1:
        return 'medium'
    return ('easy', 'medium', 'hard')[(index - 1) % 3]
