...
def run_opencode_streaming(
    *,
    ...
    timeout_s: int = 900,
    first_response_timeout_s: int = 120,
    on_event: Any = None,
    register_cancel: Any = None,
) -> Any:
    started = time.time()
    ...
    if container and host_workdir:
        try:
            ...
            error = _clean_obj({'type': 'setup_failed', 'message': str(exc)}, secrets)
            _push(error, events, on_event)
            paths = _write_logs(artifact_dir, stdout, events, secrets)
            return _result(1, session_id, events, paths, prompt_arg, error, started, time.time(), None, safe_env)

    setup_done = time.time()
    _push({'type': 'process_start', 'status': 'running', 'message': 'starting opencode process'}, events, on_event)
    proc = subprocess.Popen(
        _cmd(container, run_workdir, prompt_arg, session_id, safe_env),
    ...
    if proc.stdout:
        for line in proc.stdout:
            session, error, first = _read_line(
                line, stdout, events, session, error, first, started, secrets, on_event
            )
    ...
    if error and (not events or events[-1] is not error):
        _push(
            {'type': str(error.get('type') or 'error'), 'message': str(error.get('message') or error)},
            events,
            on_event,
        )
    if container and host_workdir:
...
def trace_payload(result: Any, repair_plan_ref: str, attempt: int) -> dict[str, Any]:
    ...
    return {
        ...
        'prompt_delivery': {
            'mode': 'file',
            'instruction': result.prompt_arg,
            'prompt_path': result.raw_paths['prompt'],
        },
        'provider': result.provider,
        'model': result.model,
        'mapping_status': _mapping_status(result, modified),
        'session_mapping': {
            'status': 'mapped' if result.session_id else 'unmapped',
            'source': 'opencode_stdout_json_events',
            'session_id': result.session_id,
        },
        'event_counts': {
            kind: sum(str(e.get('type') or 'unknown') == kind for e in result.events) for kind in event_types
        },
        'ui_events': ui,
...
def _cmd(container: str, workdir: str, prompt: str, session: str, env: dict[str, str]) -> list[str]:
    args = ['opencode', 'run', '--format', 'json', '--dangerously-skip-permissions']
...
def _opencode_env(raw: dict[str, str]) -> dict[str, str]:
    ...
    api_key = (
        raw.get('LAZYMIND_EVO_CODE_API_KEY')
        or raw.get(key_env)
        or raw.get(f'LAZYLLM_{safe}_API_KEY')
        or raw.get(f'{safe}_API_KEY')
        or ''
    )
...
def _config_cmd(env: dict[str, str]) -> str:
    provider, model, base_url = (
        env.get('OPENCODE_PROVIDER', ''),
        env.get('OPENCODE_PROVIDER_MODEL', ''),
        env.get('OPENCODE_PROVIDER_BASE_URL', ''),
    )
    key_env = env.get('OPENCODE_PROVIDER_KEY_ENV', '')
    if not (provider and model and base_url and key_env and env.get(key_env)):
        return ''
    config = {
...
def _paths(value: Any) -> list[str]:
    if isinstance(value, dict):
        return [
            ...
            for item in (
                [child] if key in {'file', 'path', 'filepath', 'filePath'} and isinstance(child, str) else _paths(child)
            )
        ]
...
def _write_logs(root: Path, stdout: list[str], events: list[dict[str, Any]], secrets: list[str]) -> dict[str, str]:
    paths = {
        'prompt': root / 'opencode_prompt.json',
        'stdout': root / 'stdout.log',
        'stderr': root / 'stderr.log',
        'events_jsonl': root / 'events.jsonl',
        'text_summary': root / 'text_summary.md',
    }
...
def _prompt_arg(prompt_path: Path, host_workdir: Path | None, workdir: str) -> str:
    ...
    return f'Read {path} as your first tool call, then apply exactly one minimal patch from that JSON task card.'
