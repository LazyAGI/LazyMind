...
def compile_intents(request: IntentRequest, intents: list[AtomicIntent]) -> IntentCompilation:
    ...
    if issues:
        return IntentCompilation([], issues)
    ordered_intents = _topological_intents(intents)
    produced_by_intent: dict[str, str] = {}
    compiled: list[CompiledIntent] = []
    for intent in ordered_intents:
        if intent.kind == 'query':
            capability_id = _query_capability_id(intent)
            operation_deps = [dep for dep in intent.depends_on if _has_plan(_by_id(intents, dep))]
            plan = IntentPlan(
                capability_id=capability_id,
                operation_id=_operation_id(intent),
                params={
                    **_query_params(intent),
                    'query_intent_id': intent.intent_id,
                    'source_message_id': request.message_id,
                },
                input_refs=[] if operation_deps else _input_refs(intent),
                required_artifact_ids=(
                    _query_artifact_ids(intent)
                    if capability_id == 'read_artifact_query' and operation_deps
                    else []
                ),
                source_message_id=request.message_id,
            )
        ...
        if intent.kind == 'chat':
            ...
            plan = IntentPlan(
                ...
                operation_id=_operation_id(intent),
                params={**intent.params, 'query_intent_id': intent.intent_id,
                        'source_message_id': request.message_id},
                source_message_id=request.message_id,
                depends_on=[],
            )
        ...
        if not late_ids and artifact_id and any(produced_by_intent.get(dep) == artifact_id for dep in operation_deps):
            late_ids = [artifact_id]
        plan = IntentPlan(
            capability_id=intent.target['capability_id'],
            operation_id=_operation_id(intent),
            params={**intent.params, 'source_message_id': request.message_id},
            input_refs=[] if late_ids else input_refs,
            required_artifact_ids=late_ids,
            source_message_id=request.message_id,
        )
    ...
    if len(ids) != len(known):
        ...
        for intent_id in ids:
            if intent_id in seen:
                issues.append(_issue(intent_id, 'duplicate_intent_id',
                                     f'duplicate intent_id: {intent_id}', 'reject'))
            seen.add(intent_id)
    for intent in intents:
        for dep in intent.depends_on:
            if dep == intent.intent_id:
                issues.append(_issue(intent.intent_id, 'self_dependency',
                                     f'intent cannot depend on itself: {intent.intent_id}', 'reject'))
            elif dep not in known:
                issues.append(_issue(intent.intent_id, 'unknown_dependency',
                                     f'unknown intent dependency: {dep}', 'reject'))
            elif _is_mutation(intent) and _by_id(intents, dep).kind == 'unsupported':
                issues.append(_issue(intent.intent_id, 'unsupported_dependency',
                                     f'mutation depends on unsupported intent: {dep}', 'reject'))
    if not issues:
        try:
            _topological_intents(intents)
        except ValueError:
            issues.append(_issue('', 'cyclic_dependency', 'intent dependencies must be a DAG', 'reject'))
    return issues
...
def _topological_intents(intents: list[AtomicIntent]) -> list[AtomicIntent]:
    ...
    children = {intent_id: [] for intent_id in by_id}
    for intent in intents:
        for dep in intent.depends_on:
            if dep not in by_id:
                continue
    ...
    if len(ordered) != len(intents):
        raise ValueError('intent dependencies must be a DAG')
    return ordered
...
def _is_mutation(intent: AtomicIntent) -> bool:
    return intent.kind in MUTATION_KINDS


def _has_plan(intent: AtomicIntent) -> bool:
    return _is_mutation(intent) or intent.kind in {'query', 'chat'}
...
def _input_refs(intent: AtomicIntent) -> list[ArtifactRef]:
    if intent.target.get('artifact_missing'):
        return []
...
def _write_artifact_id(intent: AtomicIntent) -> str:
    ...
    if params.get('output_id'):
        return str(params['output_id'])
    if str(intent.target.get('capability_id')) == 'prepare_dataset_case' and params.get('output_case_id'):
        return f"case_preparation_{params['output_case_id']}"
    if str(intent.target.get('capability_id')) == 'generate_dataset_case':
        return _dataset_case_id(params)
    fixed = {
        'load_corpus': 'corpus_load_report',
        'build_corpus_snapshot': 'corpus_snapshot',
        'assemble_dataset': 'eval_dataset',
        'assemble_classification_report': 'classification_report',
        'compare_abtest_result': 'abtest_comparison',
        'cutover_candidate_algorithm': 'candidate_algorithm_cutover',
        'build_repair_loop_plan': 'repair_loop_plan',
    }
    return fixed.get(str(intent.target.get('capability_id') or ''), '')


def _dataset_case_id(params: dict) -> str:
    if params.get('case_id'):
        return str(params['case_id'])
...
def _issue(intent_id: str, code: str, message: str, severity: str) -> ValidationIssue:
