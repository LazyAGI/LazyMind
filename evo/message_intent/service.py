...
class MessageIntentService:
    ...
    def handle(self, thread_id: str, payload: dict[str, Any], *, sync_budget_seconds: float = 3.0) -> MessageHandleResult:
    ...
    def _handle_locked(self, lease: MessageLease, turn_id: str, message_id: str, content: str) -> MessageHandleResult:
        ...
        if not pending.text.strip():
            ...
            self.store.finish_turn(lease, turn_id, status=response.status, request_fingerprint="", processed_cursor=len(content))
            return response
        cursor = 0
        intent_index = 0
        ...
        while cursor < len(pending.text):
            ...
            if not buffer.strip():
                ...
                break
            try:
                plan = self.planner.plan(buffer, message_id=_message_id_at_cursor(pending, cursor) or message_id, working_set=self._planner_context(thread_id))
            ...
            if plan.confidence < MIN_PLANNER_CONFIDENCE:
                response = self._clarify(lease, turn_id, message_id, plan.needs_clarification or "我不够确定要执行哪个 evo 操作，请更明确地描述。")
                self._finish_current_turn(lease, turn_id, content, response.status, pending, cursor)
                return response
            if consumed_len <= 0:
            ...
            context = _ExecutionContext(
                ...
                cursor,
                consumed_len,
                tuple(segment.message_id for segment in consumed_segments) or tuple(plan.consumed_message_ids or (message_id,)),
    ...
    def _finish_current_turn(
        ...
        processed_cursor = len(content) if current is None else min(len(content), max(current.processed_cursor, current.processed_cursor + max(0, min(consumed_cursor, current.end) - current.start)))
    ...
    def _mark_consumed(self, lease: MessageLease, pending: _PendingBuffer, consumed_cursor: int) -> None:
        ...
        for segment in pending.segments:
            ...
            if overlap > 0:
                updates[segment.turn_id] = max(updates.get(segment.turn_id, segment.processed_cursor), segment.processed_cursor + overlap)
        for segment in pending.segments:
    ...
    def _execute(self, lease: MessageLease, turn_id: str, message_id: str, intent: ResolvedIntent, context: _ExecutionContext) -> MessageHandleResult:
        kind = intent.kind
        if kind == "unsupported":
            return self._clarify(lease, turn_id, message_id, intent.reason or "暂不支持该 evo 操作。")
        if kind == "general_chat":
            return self._assistant(lease, turn_id, message_id, "我可以处理 evo 状态、继续/暂停/取消/重试、读取报告、查看 case 或受控修改 case 字段。", final=False)
        ...
        if kind in {"continue_flow", "pause_flow", "cancel_flow", "retry_failed", "rerun_case", "patch_artifact"} and not self.has_flow(lease.thread_id):
        ...
        if kind == "pause_flow":
            command_id = _message_command_request(turn_id, context.intent_index, kind, RUN_ID, RunControlIntent("pause"), advance_until_idle=False).command_id
            state = self.flow_getter(lease.thread_id).pause_flow(command_id=command_id, run_id=RUN_ID)
            return self._command_response(lease, turn_id, message_id, kind, {"status": state.gate_status, "current_step": state.current_step})
        if kind == "cancel_flow":
            command_id = _message_command_request(turn_id, context.intent_index, kind, RUN_ID, RunControlIntent("cancel"), advance_until_idle=False).command_id
    ...
    def _continue(self, lease: MessageLease, turn_id: str, message_id: str, context: _ExecutionContext) -> MessageHandleResult:
        flow = self.flow_getter(lease.thread_id)
        state = flow.continue_flow(command_id=flow.continue_flow_command_id(turn_id=turn_id, intent_index=context.intent_index, run_id=RUN_ID), run_id=RUN_ID)
        return self._command_response(
            lease,
            turn_id,
            message_id,
            "continue_flow",
            {"status": state.gate_status, "current_step": state.current_step, "completed_steps": list(state.completed_steps)},
        )

    def _retry_failed(self, lease: MessageLease, turn_id: str, message_id: str, context: _ExecutionContext) -> MessageHandleResult:
        command_id = _message_command_request(turn_id, context.intent_index, "retry_failed", RUN_ID, RetryFailedIntent()).command_id
        state = self.flow_getter(lease.thread_id).retry_failed_flow(command_id=command_id, run_id=RUN_ID)
        return self._command_response(lease, turn_id, message_id, "retry_failed", {"status": state.gate_status, "current_step": state.current_step})

    def _rerun_case(self, lease: MessageLease, turn_id: str, message_id: str, case_id: str, context: _ExecutionContext) -> MessageHandleResult:
        if not case_id:
            return self._clarify(lease, turn_id, message_id, "请指定要重跑的 case。")
        key = case_key("eval.rag_answer", case_id)
        command_id = _message_command_request(turn_id, context.intent_index, "rerun_case", RUN_ID, MaterializeIntent((key,), include_downstream=True)).command_id
        state = self.flow_getter(lease.thread_id).materialize_flow(
            command_id=command_id,
            run_id=RUN_ID,
            artifacts=(key,),
        )
        return self._command_response(lease, turn_id, message_id, "rerun_case", {"status": state.gate_status, "current_step": state.current_step, "case_id": case_id})

    def _patch_artifact(self, lease: MessageLease, turn_id: str, message_id: str, intent: ResolvedIntent, context: _ExecutionContext) -> MessageHandleResult:
        ...
        self.store.append_event(
            ...
            "confirmation_required",
            {
                ...
                "preview": preview,
                "provenance": {**provenance, "request_fingerprint": approval.request_fingerprint, "command_id": request.command_id},
            },
            turn_id=turn_id,
            message_id=message_id,
        )
    ...
    def _resolve_pending(self, lease: MessageLease, turn_id: str, message_id: str, kind: str, approval_token: str) -> MessageHandleResult:
        ...
        if approval.expires_at <= time.time():
            self.store.resolve_approval(lease, approval.approval_token, status="expired", event_payload={"reason": "expired"}, turn_id=turn_id, message_id=message_id)
        ...
        if stale:
            self.store.resolve_approval(lease, approval.approval_token, status="cancelled", event_payload={"reason": "stale_expected_ref", "stale_refs": stale}, turn_id=turn_id, message_id=message_id)
        ...
        self.store.resolve_approval(lease, approval.approval_token, status="approved", event_payload={}, turn_id=turn_id, message_id=message_id)
        result = self.flow_getter(lease.thread_id).runtime.execute_intent(request)
        return self._command_response(lease, turn_id, message_id, "approve_pending", {"status": result.status, "reason": result.reason})
    ...
    def _command_response(self, lease: MessageLease, turn_id: str, message_id: str, kind: str, payload: dict[str, Any], *, final: bool = False) -> MessageHandleResult:
        event = self.store.append_event(lease, "command_applied", {"kind": kind, **payload}, turn_id=turn_id, message_id=message_id)
    ...
    def _assistant(self, lease: MessageLease, turn_id: str, message_id: str, content: str, *, final: bool = True) -> MessageHandleResult:
...
def _message_command_request(
    ...
    *,
    ...
    provisional = IntentCommandRequest("msg:pending", run_id, intent, advance_until_idle=advance_until_idle, metadata=metadata or {})
    fingerprint = intent_request_fingerprint(provisional)
    return IntentCommandRequest(f"msg:{turn_id}:{intent_index}:{intent_kind}:{fingerprint}", run_id, intent, advance_until_idle=advance_until_idle, metadata=metadata or {})
