...
class ArtifactViewService:
    ...
    def _generic_facts(data: Any) -> dict[str, Any]:
        if isinstance(data, dict):
            return {"exists": True, "keys": sorted(str(key) for key in data.keys())[:30], "status": str(data.get("status") or "")}
...
def _compact_fact(value: Any) -> Any:
    if isinstance(value, dict):
        return {str(key): _compact_fact(item) for key, item in list(sorted(value.items(), key=lambda item: str(item[0])))[:MAX_FACT_ITEMS]}
