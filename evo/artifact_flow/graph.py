...
def _ops(partitions: StaticPartitions) -> tuple[type[FixedOp], ...]:
    ...
    class AssembleDataset(FixedOp):
        ...
        inputs = {"cases": ArtifactInput("eval.case", partition_spec=partitions, partition_mapping=all_to_unpartitioned())}
    ...
    class EvalSummary(FixedOp):
        op_id = "eval.summary"
        inputs = {"judges": ArtifactInput("eval.judge_result", partition_spec=partitions, partition_mapping=all_to_unpartitioned())}
    ...
    class PrepareWorkspace(FixedOp):
        ...
        @classmethod
        def execute(cls, inputs, ctx):
            return {"workspace": business.candidate_workspace(inputs["plan"].payload)}
    ...
    class RepairLoop(FixedOp):
        ...
        @classmethod
        def execute(cls, inputs, ctx):
            return {"result": business.repair_loop(inputs["workspace"].payload)}
    ...
    class CompareABTest(FixedOp):
        ...
        inputs = {"baseline": ArtifactInput("eval.summary"), "candidate": ArtifactInput("abtest.candidate_eval_summary")}
