...
export interface WindowEntry { run: RunLink; minimized: boolean; position?: { left: number; top: number }; anchor: number }
  ...
  return {
    ...
    observe(run: RunLink, anchor: number) {
      ...
      if (first === undefined || anchor < first || shouldOpen) publish({
        ...
        entries: shouldOpen ? { ...state.entries, [run.hostSessionId]: { run, minimized: false, anchor } } : state.entries,
      })
    },
    open(run: RunLink, anchor: number) { if (run.hostSessionId) update(run.hostSessionId, { run, minimized: false, anchor }) },
    minimize(id: string) { const entry = state.entries[id]; if (entry) update(id, { ...entry, minimized: true }) },
    position(id: string, position: { left: number; top: number }) { const entry = state.entries[id]; if (entry) update(id, { ...entry, position }) },
