# macOS 完整验收：定时任务、微信/飞书/企业微信与系统通知

适用日期：2026-09-17；当前独立功能分支 `codex/notifications-official-main`，源码提交 `e4c63e6`，基于官方仓库。以下全部使用最新桌面包和同一个验收目录，不再创建 Downloads 代码副本。

本次已完成：桌面包签名校验、应用启动、15 个后台服务就绪、当前管理员身份与通知配置 API 读取。**真实模型执行、系统横幅与点击、三平台消息送达仍需要按本手册人工验收。** 当前新环境自动进入本地管理员，但尚未配置模型。无需先重建桌面包；只有修改了生产源码才执行附录的重建步骤。

请按顺序执行，命令使用同一个 zsh 终端。涉及系统权限、二维码、机器人凭据、选择接收人和点击通知的步骤，终端命令不能代替实际界面操作，因此同时列出点击路径和核对命令。外部消息使用专用测试账号及同意接收的对象。没有账号、测试模型、第二用户或旧数据的项目记录“未测”。

`m`、`nreq` 复用仓库已有验收工具，不手工拷贝 Token，不绕过认证，不自动刷新/重试。结果写入验收目录；其中包含凭证、账号和正文，反馈时仅提供脱敏结果表，不整体上传目录。普通任务通常几秒至数分钟；桌面主线约 45 分钟，多平台及故障测试另需时间。每个 `m run` 标签只允许执行一次，重测请使用新的标签并同步后续文件名。
...
预期分支如上、arm64、APP_OK、签名检查成功。不要执行 reset、清库或切回已删除的旧目录。

## 2. 显示应用并确认后台就绪

当前已启动。下面命令可再次显示同一窗口；以后需要启动时也使用它：

```sh
"$LAZYMIND_ACCEPTANCE_REPO/docs/plan/notification-official-isolation/start-acceptance.command"
export LAZYMIND_ACCEPTANCE_PID="$(cat "$LAZYMIND_ACCEPTANCE_ROOT/app.pid")"
ps -p "$LAZYMIND_ACCEPTANCE_PID" -o pid=,comm=
python3 - <<'PY'
import json, os, subprocess
from pathlib import Path
r=Path(os.environ['LAZYMIND_ACCEPTANCE_ROOT'])
s=json.loads((r/'runtime/state/runtime-state.json').read_text())
p=subprocess.run([s['resourcesRoot']+'/bin/local-runtime-manager','status','--json',
    '--profile','desktop','--runtime-root',str(r/'runtime'),'--repo-root',s['repoRoot'],
    '--resources-root',s['resourcesRoot'],'--owner-token',s['ownerToken']],
    capture_output=True,text=True,timeout=30)
if p.returncode:
    raise SystemExit('后台状态读取失败，请保留现场排查。')
v=json.loads(p.stdout)
print('overall:',v.get('overallStatus'))
for name,item in v.get('services',{}).items():
    print(name, item.get('status'))
PY
```

预期 overall 为 ready，各服务 running。首次初始化尚未结束时稍候再查，不反复启动多个目录的应用。原始运行状态和日志可能含内部凭据，不直接粘贴。

## 3. 配置真实模型、检查登录和系统权限

在应用点击“去配置”，或“设置 → 模型配置”，填写你自己的可用模型配置并设为对话模型。在普通聊天发送“不要调用工具，只回复：模型可用”，必须得到真实回答。当前是本地管理员；若测试其他用户，在应用内正常登录，不能靠替换 Token 模拟。
...
预期 active、PROFILE_OK、120 秒验收令牌有效。新用户默认总开关开启、成功/失败开启、人工等待关闭、桌面开启。API 401 时先确认应用正常登录，等待原有续期或正常重新登录；不要绕过认证。保存 prefs-original 后不要在中途用当前配置覆盖它。

系统设置中进入“通知 → LazyMind”，开启通知和横幅；关闭专注模式。首次申请权限点允许。这里验收的是操作系统通知，不能用脚本生成的其他应用通知代替。2 分钟令牌仅用于本次隔离验收，不改生产默认值。
...
## 4A. 前端页面与保存行为（完成第 4 步后执行）

先打开“设置 → 消息通知”：核对总开关、成功/失败/人工等待、摘要/全文、桌面/微信/飞书/企业微信四种渠道。修改默认规则后应自动保存，刷新页面仍保留；全部关闭三个事件应提示至少选择一个。未接入的外部渠道应提示接入，不显示虚构账号或收件人。

```sh
nreq GET /api/core/user/notification-preferences 200 ui-defaults.json
jq '{enabled,revision,defaults}' "$LAZYMIND_ACCEPTANCE_ROOT/ui-defaults.json"
```

再打开“任务中心 → 定时任务”，找到“后台续期人工验收”，分别从详情、编辑或更多菜单中的“通知设置”进入：修改规则后点取消，重新进入应未保存；再次修改并明确保存后，刷新应保留。核对接口：

```sh
nreq GET "/api/core/schedules/$SID/notifications" 200 ui-task-rule.json
jq '{configured,revision,config,availability}' "$LAZYMIND_ACCEPTANCE_ROOT/ui-task-rule.json"
```

任务通知设置独立保存；保存通知后取消任务编辑，不会撤回已保存的通知，界面应明确提示。新建任务先继承当前默认规则，创建后再编辑专属通知；不要把创建弹窗里不存在的自定义规则入口列为失败。

在任务通知设置里先修改一项形成草稿，再打开“去接入”弹窗并返回：草稿应保留。接入后账号列表应刷新；选择账号后再选择实际接收人，不自动绑定第一人；切换账号应清空旧接收人。第 11–14 步有真实账号后再完成这一项。

版本冲突 UI：保持任务通知设置打开，在终端执行下方保存使版本变化，然后回页面修改并保存，应提示配置已更新，保留可重新核对的草稿，不静默覆盖服务器：

```sh
nreq GET "/api/core/schedules/$SID/notifications" 200 ui-conflict-current.json
jq '{revision,config:(.config | .channels.desktop.enabled=false)}' "$LAZYMIND_ACCEPTANCE_ROOT/ui-conflict-current.json" > "$LAZYMIND_ACCEPTANCE_ROOT/ui-conflict-request.json"
nreq PUT "/api/core/schedules/$SID/notifications" 200 ui-conflict-saved.json ui-conflict-request.json
```

重新加载后看到最新规则。结束时恢复初始任务规则和默认值，避免影响后续顺序：

```sh
nreq GET "/api/core/schedules/$SID/notifications" 200 ui-current.json
jq --slurpfile original "$LAZYMIND_ACCEPTANCE_ROOT/rule-original.json" '{revision,config:$original[0].config}' "$LAZYMIND_ACCEPTANCE_ROOT/ui-current.json" > "$LAZYMIND_ACCEPTANCE_ROOT/ui-restore.json"
nreq PUT "/api/core/schedules/$SID/notifications" 200 ui-restored.json ui-restore.json
nreq GET /api/core/user/notification-preferences 200 ui-prefs-current.json
jq --slurpfile original "$LAZYMIND_ACCEPTANCE_ROOT/prefs-original.json" '{revision,enabled:$original[0].enabled,defaults:$original[0].defaults}' "$LAZYMIND_ACCEPTANCE_ROOT/ui-prefs-current.json" > "$LAZYMIND_ACCEPTANCE_ROOT/ui-prefs-restore.json"
nreq PATCH /api/core/user/notification-preferences 200 ui-prefs-restored.json ui-prefs-restore.json
```

打开任务执行历史并选中第 4 步运行，检查对应状态、渠道、内容和时间；切换执行记录应显示各自快照，不混用当前配置。多次运行后检查刷新和加载更多不重复行。第 15 步出现失败时，同时在历史页面核对重试按钮、错误提示与 unknown 的重复投递风险确认。界面暂时没有记录时应显示空状态，不能显示成功占位结果。
...
ps -p "$LAZYMIND_ACCEPTANCE_PID" -o pid=,comm=
"$LAZYMIND_ACCEPTANCE_REPO/docs/plan/notification-official-isolation/stop-acceptance.command"
ps -p "$LAZYMIND_ACCEPTANCE_PID" -o pid=,comm=
```

确认旧 PID 已结束，再执行第 2 步的启动命令并重新记录 PID，保持所有环境变量和 profile 不变：
...
外部步骤会产生真实消息，由验收人在确认账号/对象后执行。先在应用“设置 → 终端连接”中分别选择微信、飞书并连接，用手机完成扫码/确认；使用专用测试会话发一条“通知验收，请回复收到”，等待原聊天通路正常回复。
...
在企业微信管理界面准备测试智能机器人 BotID 和 Secret，确保没有其他程序占用同一长连接。在 LazyMind“设置 → 终端连接 → 企业微信”填写凭据并点击连接，核对成功和失败提示。不要在终端命令、截图或反馈中暴露 Secret。
...
验证错误 Secret：在企业微信接入表单使用另一个专用测试 BotID 和故意错误的 Secret；预期 WECOM_AUTH_FAILED，不泄漏 SDK 异常/Secret。HTTP 201 只表示连接会话创建，不能把失败会话当作已连接成功。
...
以下仅对专用企微测试账号操作，先确认全部发送完成。在“终端连接”账号详情查看被引用的任务，点断开先取消一次应保持连接；下面命令执行实际断开。随后刷新界面应同步显示不可用：
...
使用本分支实际支持、能够稳定进入人工输入等待的测试工作流，并确保运行属于定时任务且启用 waiting 通知。没有这种测试工作流则将真实人工等待记录为未测，用第 20 步自动回归补充；不借用其他分支的工作区审批功能，不直接改数据库伪造状态。

```sh
read -r 'WAIT_TASK?输入实际进入人工输入等待的定时 task_id: '
...
人工输入等待应通知，单纯等待依赖不应发送人工等待通知。提交输入后应继续执行并最终通知，重复刷新不能重复执行任务。

普通聊天回归：在页面发送短消息、上传一份无敏感测试文件，并分别从既有飞书/微信会话触发后台聊天任务。用第 7 步命令关闭通知总开关后，原聊天提醒应保持原行为，定时任务通知才受这个开关控制。验收后重新开启总开关。不要把这次功能范围外的工作区授权功能列入验收。
...
最后 ps 应无输出；仍有进程/端口则保留现场排查。以后继续本次验收时，执行第 1、2 步恢复环境。本次 2 分钟 TTL 没有修改源码默认值。
...
```sh
(cd "$LAZYMIND_ACCEPTANCE_REPO/frontend" && pnpm exec vitest run src/modules/notifications src/modules/taskCenter/SettingsScheduleList.test.tsx src/modules/taskCenter/ScheduleList.i18n.test.tsx src/modules/taskCenter/TaskDetail.test.tsx src/modules/chat/components/TaskCenter/taskTimeline.test.ts) 2>&1 | tee "$LAZYMIND_ACCEPTANCE_ROOT/frontend-notification-tests.log"
(cd "$LAZYMIND_ACCEPTANCE_REPO/backend/core" && TEST_DB_DRIVER=sqlite go test ./scheduler -run '^TestNotification' -count=1) 2>&1 | tee "$LAZYMIND_ACCEPTANCE_ROOT/scheduler-notification-tests.log"
CHANNEL_GATEWAY_TEST_DRIVER=sqlite "$LAZYMIND_ACCEPTANCE_ROOT/test-venv/bin/python" -m pytest tests/backend/channel-gateway/test_notification_accounts.py tests/backend/channel-gateway/test_notification_boundaries.py tests/backend/channel-gateway/test_notification_contract.py tests/backend/channel-gateway/test_notification_delivery.py tests/backend/channel-gateway/test_notification_recovery.py -v --tb=short 2>&1 | tee "$LAZYMIND_ACCEPTANCE_ROOT/gateway-notification-tests.log"
```

重点对应通知写入失败不污染业务结果、旧未配置任务异步收尾、终态竞态、恢复扫描公平性和祖先重试绕过。当前官方基础功能分支桌面基线是 190 项，0 失败、0 跳过；以实际输出为准。有失败/跳过必须逐项记录。

本机 Desktop 使用 SQLite，上述不能代表 PostgreSQL/Compose 验收。若需发布两部署模式，还应按 本目录 verification.md 的专用 PostgreSQL 环境要求分别运行；本手册不连接或修改未知 PostgreSQL 实例。
...
手册命令块经过语法检查。真实消息送达、模型输出、系统横幅和点击仍以逐步人工结果为准；不要把语法检查算作产品验收。

## 附录 A. 修改源码后才需要重建

当前包已成功构建，不需要为阅读手册重建。若后续改源码，先完成所有任务，执行停止脚本，再构建：
...
LAZYMIND_RELEASE_BUILD=false LAZYMIND_DESKTOP_SIGNING_MODE=adhoc make desktop-darwin-arm64 2>&1 | tee "$LAZYMIND_ACCEPTANCE_ROOT/build.log"
codesign --verify --deep --strict "$LAZYMIND_ACCEPTANCE_APP"
"$LAZYMIND_ACCEPTANCE_REPO/docs/plan/notification-official-isolation/start-acceptance.command"
export LAZYMIND_ACCEPTANCE_PID="$(cat "$LAZYMIND_ACCEPTANCE_ROOT/app.pid")"
```

当前验证使用 Node 24。包装脚本沿用冻结 Skill 锁文件和已有缓存，并针对 Skill/Caddy 选择 Go 1.25，其余采用自动工具链；不要全局固定 GOTOOLCHAIN=go1.25.0，process-compose 需要更高版本。不清除 Skill 缓存、不删除锁文件、不用旧路径的桌面包替换新包。构建失败先停在失败处；不要继续把旧产物当成本次新构建。当前构建使用 RELEASE_BUILD=false 保留必要本地 LazyLLM。

## 附录 B. 已知边界和通过标准

全部项目须分别记录通过、失败或未测。真实旧任务升级、第二用户隔离、Windows 原生通知、完整 Docker Compose 和真实 PostgreSQL 部署不能由本机新建 SQLite 环境替代。两数据库的已有自动验证见 [verification.md](verification.md)。

当前按你的要求保留官方基线问题：SQLite aggregate 迁移既有失败、设置页单测缺失 mock，以及聊天测试文件语法错误阻塞完整 TypeScript 检查，详见 [baseline-failures.md](baseline-failures.md)。它们没有在这轮启动/文档工作中被修改，不应把全仓检查宣称为全部通过。

停止脚本只核对本验收目录的资源路径和保存的 PID，先请求后台停止，再退出主进程；后台停止失败会保留现场。执行前确认没有运行中任务。它不会删验收数据库或通知证据。本轮只实际运行过启动脚本；停止脚本已做语法检查，冷启动步骤由验收人执行验证。
