...
func manualSchemas() map[string]any {
	return map[string]any{
		...
		"ProviderStatusEvent": obj(
			prop("model_call_id", strSchema()), prop("http_status", nullableSchema(intSchema())),
			prop("finish_reason", nullableSchema(strSchema())), prop("error_body", strSchema()),
		),
		"ModelRetryEvent": obj(
			prop("model_call_id", strSchema()), prop("retry_index", intSchema()),
			prop("max_retries", intSchema()), prop("delay_ms", intSchema()),
		),
		"ModelTransportErrorEvent": obj(
			prop("model_call_id", strSchema()), prop("http_status", nullableSchema(intSchema())),
			prop("finish_reason", nullableSchema(strSchema())), prop("error_type", strSchema()),
			prop("error_message", strSchema()),
		),
		"ChatChunkResponse": obj(
			prop("conversation_id", strSchema()), prop("seq", intSchema()), prop("message", strSchema()),
			prop("delta", strSchema()), prop("finish_reason", strSchema()), prop("history_id", strSchema()),
			prop("sources", array(obj())), prop("prompt_questions", array(strSchema())),
			prop("reasoning_content", strSchema()), prop("thinking_duration_s", int64Schema()),
			prop("provider_status", refSchema("ProviderStatusEvent")), prop("model_retry", refSchema("ModelRetryEvent")),
			prop("model_transport_error", refSchema("ModelTransportErrorEvent")),
		),
