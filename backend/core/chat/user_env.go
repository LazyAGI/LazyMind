	...
	for index := len(histories) - 1; index >= 0; index-- {
		...
		err := db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			...
			if choice == "__ask_user_yes__" {
				result := tx.Where("id = ? AND user_id = ? AND name = ? AND updated_at = ?",
					target.ID, owner, target.Name, target.ExpectedUpdatedAt).Delete(&orm.UserEnvironmentVariable{})
				if result.Error != nil {
					return result.Error
				}
				if result.RowsAffected != 1 {
					return errors.New("environment variable changed or is unavailable; request a new deletion confirmation")
				}
