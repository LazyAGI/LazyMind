...
func UpdateGroupPlacement(w http.ResponseWriter, r *http.Request) {
	...
	err := UserTransaction(r.Context(), store.DB(), uid, func(tx *gorm.DB) error {
		...
		if err := tx.Where("user_id=? AND deleted_at IS NULL AND pinned=? AND id<>? AND kind=? AND is_task_conv=?", uid, pinned, id, current.Kind, current.IsTaskConv).Order("sort_order ASC, created_at ASC, id ASC").Find(&groups).Error; err != nil {
			return err
		}
		index := 0
