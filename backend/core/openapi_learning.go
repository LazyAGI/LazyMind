...
func learningOpenAPISchemas() map[string]any {
	...
	return map[string]any{
		...
		"LearningSelectionRequest":        objReq([]string{"dataset_id", "text"}, prop("dataset_id", strSchema()), prop("text", strSchema())),
		...
		"LearningContentConfirmRequest":   objReq([]string{"value", "book_ids"}, prop("value", obj()), prop("book_ids", array(strSchema()))),
		...
		"LearningPreanalysisRequest":      objReq([]string{"dataset_id", "document_id", "capability_keys", "items"}, prop("dataset_id", strSchema()), prop("document_id", strSchema()), prop("document_revision", strSchema()), prop("capability_keys", array(strSchema())), prop("items", array(obj()))),
...
func learningOpenAPIPaths() map[string]any {
	return map[string]any{
		...
		"/learning/presets:resolve":                            map[string]any{"get": learningOperation("Resolve a document, knowledge-base, or global preset", nil, nil)},
		"/learning/selections:analyze":                         map[string]any{"post": learningOperation("Analyze selection language, granularity, dictionary matches, capabilities, and compatible books", nil, refSchema("LearningSelectionRequest"))},
		...
		"/learning/content/{content_id}:confirm":               map[string]any{"post": learningOperation("Confirm edited schema content and attach compatible learning books", lp("content_id"), refSchema("LearningContentConfirmRequest"))},
		...
		"/learning/review/sessions/{session_id}:complete":      map[string]any{"post": learningOperation("Complete a review session without grading unanswered items", lp("session_id"), nil)},
