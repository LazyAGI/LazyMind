	...
	for _, driver := range []string{"sqlite", "postgres"} {
		t.Run(driver, func(t *testing.T) {
			...
			for _, migration := range catalog.Modes[len(catalog.Modes)-1].Dev {
				...
				case 20260908090000, 20260909022313, 20260909045218, 20260909083928:
					groupMigrations = append(groupMigrations, migration)
				default:
					execMigrationFileForDriver(t, db, migration.UpPath, driver)
				}
			}
			if len(groupMigrations) != 4 {
				t.Fatalf("expected four conversation group migrations, got %d", len(groupMigrations))
			}
