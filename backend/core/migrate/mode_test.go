...
func TestRepositoryStructuredMigrationCatalogLoads(t *testing.T) {
	...
	if len(v03.Dev) != 82 {
		t.Fatalf("v0_3 dev migration count=%d, want 82", len(v03.Dev))
	}
	...
	for _, version := range []uint64{20260914090000, 20260909073741, 20260909193000, 20260910080000, 20260912063102, 20260912065741} {
		if !containsMigrationFileVersion(v03.Dev, version) {
			t.Fatalf("v0_3 dev migrations are missing %d", version)
		}
	}
