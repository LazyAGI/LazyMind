...
func TestRepositoryStructuredMigrationCatalogLoads(t *testing.T) {
	...
	for _, token := range []string{"enable_plugin_was_null", "DROP TABLE IF EXISTS chat_run_performance", "DROP TABLE IF EXISTS conversation_policy_snapshot_backups", "DROP COLUMN IF EXISTS pinned_at", "DROP COLUMN IF EXISTS free_auto_select_priority", "DROP COLUMN IF EXISTS free_auto_select_base_urls", "DROP COLUMN IF EXISTS source_context", "DROP COLUMN IF EXISTS source_selected_text", "DROP COLUMN IF EXISTS source_seq", "DROP COLUMN IF EXISTS source_history_id", "DROP COLUMN IF EXISTS relation_type", "DROP COLUMN IF EXISTS parent_conversation_id", "DROP COLUMN IF EXISTS chat_model_version", "DROP COLUMN IF EXISTS chat_model_snapshot", "DROP COLUMN IF EXISTS chat_model_id", "DROP COLUMN IF EXISTS chat_model_mode", "idx_conversations_parent_relation", "ON public.skills(owner_user_id, category, skill_name)", "AS identity_conflicts", "AS relative_root_conflicts", "performance_stats_enabled"} {
