...
func routeEventWithWorkflowHooks(ctx context.Context, db *gorm.DB, stateStore state.Store, ev TaskEvent, artifactHook, terminalHook bool) error {
	...
	case "done":
		...
		accepted, err := acceptFinalStatusWithPhase(ctx, db, ev.TaskID, status, ev.Summary, ev.CurrentPhase)
		if err != nil {
	...
	case "error":
		...
		accepted, err := acceptFinalStatusWithPhase(ctx, db, ev.TaskID, status, ev.Message, ev.CurrentPhase)
		if err != nil {
		...
		fields := map[string]any{"status": status, "summary": ev.Message}
		if ev.CurrentPhase != "" {
			fields["current_phase"] = ev.CurrentPhase
		}
		_ = WriteStatus(ctx, stateStore, ev.TaskID, fields)
