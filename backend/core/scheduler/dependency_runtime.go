...
func launchDependentTask(db *gorm.DB, s orm.UserSchedule, taskID, contextText string) {
	...
	reqBody := map[string]any{"query": query, "conversation_id": convID, "stream": true, "mode": "auto", "input": []map[string]any{{"input_type": "text", "text": query}}, "skip_sensitive_filter": true}
