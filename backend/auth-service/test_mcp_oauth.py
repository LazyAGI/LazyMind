...
def _process_token(database_url, args, counter, queue):
    ...
    def request(url, data=None, json_body=None):
        with counter.get_lock():
            counter.value += 1
        time.sleep(.2)
        return {'access_token': 'process-token', 'refresh_token': 'rotated', 'token_type': 'Bearer', 'expires_in': 3600}
...
class MCPOAuthTests(unittest.TestCase):
    def setUp(self):
        ...
        def request(url, data=None, json_body=None):
            ...
            if 'well-known' in url:
                return {'issuer': 'https://auth.example', 'authorization_endpoint': 'https://auth.example/authorize', 'token_endpoint': 'https://auth.example/token', 'registration_endpoint': 'https://auth.example/register', 'code_challenge_methods_supported': ['S256']}
            ...
            if data.get('grant_type') == 'refresh_token':
                ...
                if self.hook:
                    self.hook()
            return {'access_token': 'token-' + str(self.refreshes), 'refresh_token': 'refresh', 'token_type': 'Bearer', 'expires_in': 3600}
    ...
    def test_replay_and_owner_isolation(self):
        ...
        with self.assertRaises(OAuthError):
            self.service.token(**dict(self.identity, user_id='bob'), grant_id=grant['grant_id'], grant_version=grant['grant_version'])
    ...
    def test_parallel_refresh_one_exchange_and_stale_grants(self):
        from services.mcp_oauth import OAuthError
        grant = self.authorize()
        args = dict(self.identity, grant_id=grant['grant_id'], grant_version=grant['grant_version'], rejected_token_version=1)
    ...
    def test_disconnect_wins_refresh_race(self):
        ...
        with self.assertRaises(OAuthError):
            self.service.token(**self.identity, grant_id=grant['grant_id'], grant_version=grant['grant_version'], rejected_token_version=1)
    ...
    def test_multiprocess_refresh_uses_database_lease(self):
        ...
        processes = [ctx.Process(target=_process_token, args=(self.engine.url.render_as_string(hide_password=False), args, counter, queue)) for _ in range(4)]
