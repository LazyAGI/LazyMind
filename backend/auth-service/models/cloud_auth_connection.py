...
class CloudAuthConnection(Base):
    ...
    connection_method = mapped_column(String(32), nullable=False, default='legacy_byo', comment='managed_oauth/cli_personal_app/legacy_byo')
    credential_location = mapped_column(String(16), nullable=False, default='local', comment='cloud/local/cli_sidecar')
    profile_ref = mapped_column(String(255), nullable=False, default='', comment='Non-sensitive local CLI profile reference')
    cloud_connection_id = mapped_column(String(64), nullable=True, index=True, comment='LazyCloud connection reference')
    cloud_owner_user_id = mapped_column(String(64), nullable=False, default='', index=True, comment='LazyCloud owner mapping')
    ...
    capability_contract_version = mapped_column(String(64), nullable=False, default='', comment='Provider capability contract')
