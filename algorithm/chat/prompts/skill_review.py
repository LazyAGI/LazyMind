...
def skill_extraction_gate_prompt(trajectory: dict) -> str:
    return f"""
...
def contextual_description_prompt(trajectory: dict) -> str:
    return f"""
...
def refined_trajectory_prompt(trajectory: dict) -> str:
    return f"""
...
def resolution_prompt(candidate: dict[str, Any], called_skills: dict[str, str]) -> str:
    return (
        ...
        '- suggestion: a user-reviewable reason for the decision; for patch, explain where to modify and how; for new, explain why it should be a separate skill\n'
