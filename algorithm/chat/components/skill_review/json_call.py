...
def call_json(llm, prompt: str, schema: Any, *, max_retries: int = 3) -> dict[str, Any]:
...
def _record_token_usage(prompt: str, raw: Any) -> None:
    ...
    LOG.info(f'Total input token chars: {TOTAL_INPUT_TOKEN_CHARS}, total output token chars: {TOTAL_OUTPUT_TOKEN_CHARS}')
