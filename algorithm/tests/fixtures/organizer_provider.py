    ...
    def do_POST(self):
        ...
        if '严格按response_schema只输出JSON。输入：\n' in text:
            ...
            if payload.get('mode') == 'scope_audit':
                ...
                operations = [] if target else [
                    {'op': 'create', 'id': 'cand_work', 'name': '工作任务', 'scope': '处理日常邮件和工作任务'}]
                result = {'candidate_operations': operations, 'assignments': [
                    {'id': x['id'], 'group_id': target or 'cand_work'} for x in payload['conversations']]}
        else:
