...
def sync_writer_documents(
    ...
    if source.document_id != revised.document_id:
        raise ToolInvalidArgumentsError('WriterDocument document_id values must match.')
    for field in ('stage', 'revision', 'provider_binding'):
        if getattr(source, field) != getattr(revised, field):
            raise ToolInvalidArgumentsError(f'WriterDocument {field} values must match.')
...
def _feishu_url(user_input: str) -> str:
    ...
    if not match:
        raise ToolInvalidArgumentsError('A Feishu/Lark document URL is required.')
...
class WriterToolkitBase:
    ...
    def build_resources(
        ...
        if not isinstance(file_paths, list):
            raise ToolInvalidArgumentsError('file_paths_json must be a JSON array.')
        resources = [{
    ...
    def resolve_visual_needs(
        ...
        if allowed_strategies is not None and not isinstance(allowed_strategies, list):
            raise ToolInvalidArgumentsError('allowed_strategies_json must contain a JSON list.')
        result = WriterMultimodalTools(
    ...
    def profile_resources(self, writing_task_json: str, user_input: str, resources_json: str = '[]') -> str:
        ...
        if not isinstance(resources, list):
            raise ToolInvalidArgumentsError('resources_json must be a JSON array.')
    ...
    def load_document(self, user_input: str, stage: str = 'final') -> str:
        ...
        if stage not in {'outline', 'draft', 'final'}:
            raise ToolInvalidArgumentsError('stage must be outline, draft, or final.')
        root = _temp_root()
    ...
    def _write_document(
        ...
        *,
        ...
        if target is None:
            raise ToolInvalidArgumentsError('A target provider document is required.')
