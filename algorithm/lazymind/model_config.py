...
_INNER_CONFIG_PATH = _RESOURCE_DIR / 'runtime_models.inner.yaml'
_ONLINE_CONFIG_PATH = _RESOURCE_DIR / 'runtime_models.online.yaml'
_DYNAMIC_CONFIG_PATH = _RESOURCE_DIR / 'runtime_models.yaml'
...
def get_dynamic_model_configs() -> Optional[Dict[str, Any]]:
    '''Return the current per-request dynamic model config dict, if available.'''
    try:
        import lazyllm
        return lazyllm.globals.config['dynamic_model_configs']
    except Exception:
        return None
...
def is_role_enabled(
    ...
    dynamic_model_configs: Optional[Dict[str, Any]] = None,
) -> bool:
    '''Return whether a model role is currently usable under the active config.

    Non-dynamic roles are always enabled because they can be constructed directly
    from the active config file. Dynamic roles are enabled only when the current
    request provides the required slot config for that role.
    '''
    resolved_config_path = config_path or get_config_path()
    role_slots = get_dynamic_role_slot_map(resolved_config_path)
    if role not in role_slots:
        return True

    cfg = dynamic_model_configs if dynamic_model_configs is not None else get_dynamic_model_configs()
    if not isinstance(cfg, dict):
        return False
    ...
    return bool(role_cfg.get(role_slots[role]))


def get_enabled_role_config_path(role: str) -> Optional[str]:
    '''Return the active config path when a role is enabled, else None.'''
    config_path = get_config_path()
    return config_path if is_role_enabled(role, config_path=config_path) else None
