...
@router.get('', summary='List all algorithms with their global instance health')
async def list_algorithms():
    ...
    result = []
    for algo in algos:
        instances = registry.get_all_instances(algo.id)
        result.append({
            'id': algo.id,
            'name': algo.name,
...
@router.post('/{algorithm_id}/restart', summary='Restart local instances of an algorithm')
async def restart_algorithm(algorithm_id: str, req: RestartRequest):
    pm = get_process_manager()

    if req.port is not None:
        await pm.restart_instance(pm.host, req.port)
        return {'restarted_ports': [req.port]}

    # Restart all local instances of this algorithm
    async with AsyncSessionLocal() as session:
