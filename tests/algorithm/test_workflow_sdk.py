...
def test_controlled_sdk_claims_existing_execution_and_preserves_control():
    ...
    receipt = {'command_id': 'complete:attempt-1'}
    ...
    result = client.complete_execution('run 1', 'attempt/1', 'current-handle', outcome='succeeded')
...
def test_controlled_sdk_does_not_send_missing_execution_handle():
    ...
    with pytest.raises(WorkflowClientError):
        client.complete_execution('run', 'attempt', '', outcome='succeeded')
    transport.post.assert_not_called()


def test_sdk_publishes_before_completing_without_resending_outputs():
    from lazymind.workflow_sdk import WorkflowClient
    transport = MagicMock()
    transport.post.return_value = MagicMock(status_code=200, json=lambda: {'ok': True, 'result': {}})
    client = WorkflowClient('http://core/api/core', 'owner', transport=transport)
    artifact = {'slot': 'pages', 'seq': 3, 'content_type': 'text', 'value': {'text': 'page'}}
    client.publish_artifact('run', 'attempt', 'handle', artifact)
    published = transport.post.call_args
    assert published.args[0].endswith('/hosted-attempts/attempt/artifacts')
    assert published.kwargs['json'] == {'artifact': artifact, 'execution_handle': 'handle'}
    client.complete_execution('run', 'attempt', 'handle', outcome='succeeded')
    assert transport.post.call_args.args[0].endswith('/hosted-attempts/attempt:complete')
    assert 'artifacts' not in transport.post.call_args.kwargs['json']
