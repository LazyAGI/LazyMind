...
from lazymind.chat.service.integrations.remote_fs import RemoteFS
...
def test_remote_fs_uses_core_readonly_api(monkeypatch):
    ...
    monkeypatch.setattr('lazymind.chat.service.integrations.remote_fs.requests.get', fake_get)
    monkeypatch.setattr('lazymind.chat.service.integrations.remote_fs.lazyllm.globals', {'agentic_config': {'session_id': 'sid-1'}})
...
def test_remote_fs_omits_session_id_when_not_available(monkeypatch):
    ...
    monkeypatch.setattr('lazymind.chat.service.integrations.remote_fs.requests.get', fake_get)
    monkeypatch.setattr('lazymind.chat.service.integrations.remote_fs.lazyllm.globals', {'agentic_config': {}})
...
def test_skill_manager_lists_remote_skills_from_mock_server(monkeypatch, mock_remote_fs_server):
    FS._instances.clear()
    monkeypatch.setattr('lazymind.chat.service.integrations.remote_fs.lazyllm.globals', {'agentic_config': {'session_id': 'sid-demo'}})
