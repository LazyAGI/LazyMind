...
|------|------|
| 标准启动 | `make up` |
| 无 Redis 本地栈（SQLite 状态后端） | `make up-build LAZYMIND_STATE_BACKEND=sqlite` |
...
`LAZYMIND_STATE_BACKEND=sqlite` 会把 chat/auth/subagent 等短生命周期状态写入 SQLite，并跳过 Redis compose profile。不设置该变量，或设置为 `LAZYMIND_STATE_BACKEND=redis`，则使用默认 Redis 状态后端。
