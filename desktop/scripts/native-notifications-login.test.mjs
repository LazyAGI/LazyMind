...
function fixture() {
  ...
  let stored = null;
  window.localStorage = { getItem: (key) => { assert.equal(key, "lazymind:user"); return stored; } };
  const calls = [];
  const bridge = {
...
test("new preload replaces stale stored tokens before the application reads auth", () => {
  ...
  let stored = JSON.stringify(user);
  ...
  assert.equal(JSON.parse(stored).token, "synthetic-background");
  assert.equal(JSON.parse(stored).refreshToken, "synthetic-rotated");
  assert.equal(JSON.parse(stored).username, user.username);
});
