...
完整数据会保存为 `./hot_lists.json` 文件，同时在 stdout 的
`LAZYMIND_RESULT_JSON_BEGIN` 与 `LAZYMIND_RESULT_JSON_END` 标记之间输出同一份 JSON。
这使 LazyMind 等脚本运行目录不可持续访问的平台也能直接取得结果。结果包含：
- `version`: "2.0"
- `timestamp`: 抓取时间
...
优先读取 stdout 标记之间的 JSON；若运行环境能够持续访问脚本目录，也可以读取
`./hot_lists.json`。两处内容相同，脚本已自动完成评分，智能体需要：
