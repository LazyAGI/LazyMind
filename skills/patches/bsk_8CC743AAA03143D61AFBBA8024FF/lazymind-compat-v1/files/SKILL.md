...
In LazyMind, use `agentic-coding/` within the current conversation workspace. Read `setup.md` only when essential project context is missing. A complete coding request authorizes its scoped implementation; briefly state the contract and proceed, without an extra approval turn. Long-term persistence outside the conversation remains opt-in.

## LazyMind execution

Use available workspace tools for code and artifacts. Load only the relevant reference for the current step. Use one compact PACT contract, one focused change and one handoff; do not repeatedly rewrite evidence notes. Run tests when an actual execution tool is available. `run_script` executes only scripts inside this Skill, not arbitrary workspace files; this package provides no general code runner. If execution is unavailable, deliver the code, reproduction and test commands, clearly label them **not executed**, and identify residual compatibility risks. Never present arithmetic reasoning as executed tests.
