...
import { browserNotificationsAuthorized, browserNotificationsSupported } from './browser';
import './index.scss';
...
export default function RuleEditor({ value, onChange, disabled = false, variant = 'settings' }: {
  ...
  const [desktopAuthorized, setDesktopAuthorized] = useState(() => isDesktopRuntime() || browserNotificationsAuthorized());
  ...
  useEffect(() => {
    const update = () => setDesktopAuthorized(isDesktopRuntime() || browserNotificationsAuthorized());
  ...
  const channelView = <div className="notification-channels">
    ...
    {variant === 'settings' && !loading && !error && !accounts.some(a => a.status === 'connected') && <Alert type="warning" showIcon message={t('notifications.noExternalChannels')} action={<Button disabled={disabled} onClick={() => openConnection()}>{t('notifications.connectTitle')}</Button>} />}
    <section className="notification-surface">{channels.map(channel => {
      ...
      return <div key={channel} className={`notification-channel-block is-${channel}`}><div className="notification-row notification-channel">
        ...
        <div className="notification-grow"><strong>{t('notifications.' + channel)}</strong> <Tag className={`notification-status is-${channel === 'desktop' ? desktopAuthorized ? 'connected' : 'disconnected' : available ? 'connected' : 'disconnected'}`}>{t('notifications.' + (channel === 'desktop' ? desktopAuthorized ? 'authorized' : 'notAuthorized' : available ? 'connected' : 'notConnected'))}</Tag>
