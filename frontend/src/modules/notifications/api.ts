...
import type { ChannelAccount, ChannelProvider } from '@/modules/channelGateway/api';
...
export interface Target { recipient_id: string; label: string; available: boolean }
...
export interface AccountDetail extends ChannelAccount {
  avatar_url?: string | null; primary_recipient: Target | null; notification_reference_count: number;
}
...
export const putScheduleNotifications = async (scheduleId: string, revision: number, config: NotificationConfig) =>
  unwrap<ScheduleNotifications>((await axiosInstance.put(`${core}/schedules/${id(scheduleId)}/notifications`, { revision, config })).data);
...
export const getAttempts = async (taskId: string, cursor = ''): Promise<Page<Attempt>> =>
  (await axiosInstance.get(`${gateway}/task-notifications`, { params: { task_id: taskId, cursor, limit: 100 } })).data;
...
export const getTargets = async (accountId: string, cursor = ''): Promise<Page<Target>> =>
  (await axiosInstance.get(`${gateway}/channel-accounts/${id(accountId)}/notification-targets`, { params: { cursor, limit: 100 } })).data;
