...
import DocumentLearningPanel, { displayPresetKey, presetDisplayValue } from "./DocumentLearningPanel";
...
it("sends a user focus and only the selected paragraphs for local analysis",async()=>{
  ...
  render(<DocumentLearningPanel datasetId="ds" documentId="doc" revision="r1" capabilities={[capability]} localAvailable/>);
  fireEvent.click(await screen.findByRole("button",{name:/AI 分析文档/}));
  fireEvent.click(screen.getByText("指定段落"));
  fireEvent.change(screen.getByPlaceholderText(/粘贴或编辑要分析的段落/),{target:{value:"第一段\n\n第二段"}});
  ...
  await waitFor(()=>expect(api.createPreanalysisTask).toHaveBeenCalledWith(expect.objectContaining({
    ...
    items:[{text:"第一段",context:"第一段",page:undefined},{text:"第二段",context:"第二段",page:undefined}],
  })));
});
...
it("restores a partially successful task and exposes all successful drafts",async()=>{
  ...
  render(<DocumentLearningPanel datasetId="ds" documentId="doc" revision="r1" capabilities={[capability]} localAvailable/>);
  await waitFor(()=>expect(screen.getByRole("button",{name:/AI 分析文档/})).toBeEnabled());
  expect(await screen.findByText("待确认回答（1）")).toBeInTheDocument();
...
it("publishes selected drafts from the side panel and keeps remaining drafts visible",async()=>{
  ...
  render(<DocumentLearningPanel datasetId="ds" documentId="doc" revision="r1" capabilities={[capability]} localAvailable/>);
  expect(await screen.findByText("待确认回答（2）")).toBeInTheDocument();
  fireEvent.click(screen.getByRole("checkbox",{name:/急弯/}));
  fireEvent.click(screen.getByRole("button",{name:"确认加入（1）"}));
...
it("stops a running analysis and restores the analysis action",async()=>{
  ...
  render(<DocumentLearningPanel datasetId="ds" documentId="doc" revision="r1" capabilities={[capability]} localAvailable/>);
  fireEvent.click(await screen.findByText("停止分析"));
  const stopButtons=await screen.findAllByRole("button",{name:"停止分析"});
  ...
  await waitFor(()=>expect(screen.queryByRole("button",{name:"停止分析"})).not.toBeInTheDocument());
  expect(screen.getByRole("button",{name:/AI 分析文档/})).toBeEnabled();
});
