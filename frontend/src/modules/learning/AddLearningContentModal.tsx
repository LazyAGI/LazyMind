...
export default function AddLearningContentModal({value,datasetId,documentId,segmentId,context,onClose,onAdded}:Props){
  ...
  useEffect(()=>{ if(!request)return; let active=true; setCapability(undefined);setBooks([]);setSource("");setResolvedValue({});setEditing(false);setLoading(true);setDictionaryMeaning(""); Promise.all([getLearningCatalog(),listLearningBooks()]).then(async ([catalog,allBooks])=>{ const def=catalog.capabilities.find(x=>x.key===request.capability_key); if(!def)throw new Error(t("learning.unsupportedCapability")); const compatible=allBooks.filter(x=>x.capability_key===def.key); const resolved=await resolveLearningContent({...request,preview:true}); if(!active)return; setCapability(def);setBooks(compatible);setSource(resolved.source);setResolvedValue(resolved.value);setDictionaryMeaning(String(resolved.value.dictionary_meaning||"")); }).catch(e=>{if(active)message.error(getLocalizedErrorMessage(e))}).finally(()=>{if(active)setLoading(false)}); return()=>{active=false}; },[request,t]);
