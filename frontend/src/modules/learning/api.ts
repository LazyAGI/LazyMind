...
export interface AnalysisConfig { instruction:string; resolution_instruction:string; extraction_prompt_template:string; resolution_prompt_template:string; allow_plain_text_single_field:boolean; max_candidates:number; max_document_candidates:number; generated_required_fields:string[]; output_language:string; fallback_pattern:string; fallback_kinds:string[]; language_aliases:Record<string,string>; subject_kind_aliases:Record<string,string> }
...
export const createPreanalysisTask = async (value:{dataset_id:string;document_id:string;document_revision?:string;capability_keys:string[]}) => normalizeTask((await axiosInstance.post(`${root}/preanalysis/tasks`,value,{silentError:true} as never)).data.data);
