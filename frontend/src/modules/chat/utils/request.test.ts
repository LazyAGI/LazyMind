...
describe('TaskServiceApi.listConversationArtifacts', () => {
  ...
  it('requests optional v2 projection through the backward-compatible endpoint', async () => {
    const response = { data: { data: { artifacts: [] } } };
    getMock.mockResolvedValueOnce(response);

    await expect(
      TaskServiceApi().listConversationArtifacts('conversation/1'),
    ).resolves.toBe(response);
    expect(getMock).toHaveBeenCalledWith(
      '/api/core/conversations/conversation%2F1/artifacts?projection=v2',
      undefined,
    );
  });
});
