...
export default function ConversationGroupPage() {
  ...
  return <main className="conversation-group-page">
    ...
    <section className="conversation-group-page-list">
      ...
      {conversations.length === 0 ? <div className="conversation-group-page-empty"><Empty description={t("conversationOrganizer.empty")} /><Button type="primary" icon={<PlusOutlined />} onClick={startChat}>{t("conversationOrganizer.startFirst")}</Button></div> : conversations.map((conversation) => <div className="conversation-group-page-item" key={conversation.conversation_id} draggable={group.kind !== "project"} onDragStart={e => startConversationDrag(e, conversation.conversation_id, group.id)}>
        ...
        <ConversationMembership pinned={Boolean(conversation.pinned_at)} conversationId={conversation.conversation_id} groupId={group.id} groupKind={group.kind} title={conversation.display_name} />
      </div>)}
