...
import { emitConversationGroupsChanged, removeConversation } from "./api";
...
export default function ConversationMembership({ conversationId, groupId, title, disabled = false, pinned = false }: {
  ...
  return <>
    <Dropdown trigger={["click"]} menu={{ items: [
      ...
      { key: "move", label: t("conversationOrganizer.adjustMembership"), disabled, onClick: () => setOpen(true) },
      ...(groupId ? [{ key: "remove", label: t("conversationOrganizer.removeFromGroup"), disabled, onClick: async () => { await removeConversation(groupId, conversationId); emitConversationGroupsChanged(); } }] : []),
