import { FolderOpenOutlined, MessageOutlined, PlusOutlined, EllipsisOutlined, PushpinOutlined } from "@ant-design/icons";
...
type Props = { namesLocked?: boolean; groups: ConversationGroup[]; searchText?: string; currentConversationId?: string; onNew?: (id: string) => void; onEdit: (group: ConversationGroup | "new" | "new-project") => void; onRemove: (group: ConversationGroup) => void };
export default function SidebarGroups({ groups, searchText = "", currentConversationId, onNew, onEdit, onRemove, namesLocked = false }: Props) {
  ...
  const block = (g: ConversationGroup) => {
    ...
    return <div key={g.id} className={`conversation-group ${dropTarget === g.id ? "group-drop-target" : ""}`} onDragOver={e => { if (!searchText && !busy && (g.kind !== "project" && e.dataTransfer.types.includes(CONVERSATION_DRAG) || e.dataTransfer.types.includes(GROUP_DRAG))) { e.preventDefault(); setDropTarget(g.id); } }} onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setDropTarget(""); }} onDrop={async e => {
      ...
      try {
        ...
        else if (g.kind !== "project" && conversation && !groups.some(group => group.id === conversation.groupId && group.kind === "project") && conversation.groupId !== g.id) { await assignConversation(g.id, conversation.id); emitConversationGroupsChanged(); }
      ...
      {open && <div className="conversation-group-members">
        {visible.map(c => <div key={c.conversation_id} className={`conversation-group-member ${c.conversation_id === currentConversationId ? "active" : ""}`} draggable={!searchText && g.kind !== "project"} onDragStart={e => startConversationDrag(e, c.conversation_id, g.id)}>
          <button title={c.display_name} onClick={() => navigate(getChatConversationPath(c.conversation_id))}>{c.display_name || c.conversation_id}</button><ConversationMembership pinned={Boolean(c.pinned_at)} conversationId={c.conversation_id} groupId={g.id} groupKind={g.kind} title={c.display_name} />
        </div>)}
  ...
  return <>
    ...
    <div className="conversation-groups-heading"><span>{t("conversationOrganizer.groups")}</span><Dropdown trigger={["click"]} menu={{ items: [
