...
describe("organizer steps", () => {
  it("shows four steps and keeps scope review within organization", () => {
    const { unmount } = render(<OrganizerSteps run={run} />);
    for (const id of ["preparation", "organization", "review", "application"]) expect(screen.getByText(`conversationOrganizer.steps.${id}`)).toBeTruthy();
