...
const api = vi.hoisted(() => ({ getLatestSession: vi.fn(), listDismissedSessions: vi.fn(), snapshot: vi.fn(), watches: new Map<string, () => void>() }));
vi.mock('@/modules/chat/utils/loadWorkflowRun', () => ({
  loadWorkflowRunSnapshot: api.snapshot,
  watchWorkflowRun: (id: string, callback: () => void) => {
    api.watches.set(id, callback);
    return () => api.watches.delete(id);
  },
}));
...
beforeEach(() => {
  vi.clearAllMocks();
  api.getLatestSession.mockResolvedValue({ data: { data: { session: session() } } });
  api.listDismissedSessions.mockResolvedValue({ data: { data: { sessions: [] } } });
  useWorkflowStore.setState({ sessionByConversation: {}, autoRunningByConversation: {} });
});
...
describe('versioned workflow synchronization', () => {
  ...
  it('reopens only on a newer snapshot and clears a stale automatic-running flag at completion', () => {
    ...
    store.setSession('c', { ...session(12), status: 'completed', projection: { completed: true } });
    ...
    expect(useWorkflowStore.getState().sessionByConversation.c?.status).toBe('completed');
    store.setSession('c', { ...session(13), status: 'active' });
  ...
  it('does not let a REST load started before completion overwrite the live snapshot', async () => {
    ...
    api.snapshot.mockImplementationOnce(() => new Promise((done) => { resolve = done; }));
    const store = useWorkflowStore.getState();
    store.setSession('c', session(8));
    const loading = store.loadActiveSession('c');
    await vi.waitFor(() => expect(resolve).toBeDefined());
    store.setSession('c', { ...session(12), status: 'completed', projection: { completed: true } });
    resolve({ session: session(8) });
    await loading;
    expect(useWorkflowStore.getState().sessionByConversation.c?.status).toBe('completed');
    expect(useWorkflowStore.getState().sessionByConversation.c?.state_version).toBe(12);
  });

  it('keeps completion when an event bell reloads an older snapshot', async () => {
    const store = useWorkflowStore.getState();
    store.setSession('c', session(8));
    api.snapshot.mockResolvedValue({ session: { ...session(8), status: 'active' } });
    await store.loadActiveSession('c');
    store.setSession('c', { ...session(12), status: 'completed', projection: { completed: true } });
    api.watches.get('s')!();
    await vi.waitFor(() => expect(api.snapshot).toHaveBeenCalledTimes(2));
    expect(useWorkflowStore.getState().sessionByConversation.c?.status).toBe('completed');
    expect(useWorkflowStore.getState().sessionByConversation.c?.projection?.completed).toBe(true);
  });
});
