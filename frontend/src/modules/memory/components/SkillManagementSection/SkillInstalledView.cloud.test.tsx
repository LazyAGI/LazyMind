...
describe("Desktop 我的技能 local and Cloud integration", () => {
  it.each(["desktop", "cloud", "local"].flatMap((mode) =>
    ...
    const navigation = screen.getByRole("navigation", { name: translation("admin.memorySkillViewBarLabel") });
    expect(within(navigation).getAllByRole("button")).toHaveLength(3);
    expect(screen.queryAllByRole("tab", { name: /^(本地|云端)$/ })).toHaveLength(0);
    if (mode === "desktop" && state === "signed_in") {
      ...
      expect(screen.queryByRole("menuitem", { name: translation("admin.memoryCloudUploadAction") })).not.toBeInTheDocument();
    }
  ...
  it.each(["desktop", "cloud", "local"].flatMap((mode) =>
    ...
    expect(await screen.findByText("local-only", { exact: true })).toBeVisible();
    expect(screen.getByRole("button", { name: "我的技能" })).toHaveAttribute("aria-current", "page");
    expect(screen.queryAllByRole("tab", { name: /^(本地|云端)$/ })).toHaveLength(0);
    expect(screen.queryByText("登录 LazyMind Cloud 后可查看和下载自己的云端资源")).not.toBeInTheDocument();
    if (mode !== "desktop") expect(mocks.session).not.toHaveBeenCalled();
  });
