...
export default function CloudResourceDetail({ resourceType }: { resourceType: CloudResourceType }) {
  ...
  if (!isDesktopRuntime()) return <Navigate to={back} replace />;
  if (signedOut) return <Navigate to={back} replace />;
  ...
  return <div className="cloud-resource-detail">
    <header className="cloud-resource-detail-header">
      <Space wrap><Button icon={<ArrowLeftOutlined />} onClick={() => navigate(back)}>{t("common.back")}</Button><h1>{metadata?.resource_name || t("admin.memoryCloudViewDetail")}</h1><Tag color="blue">{t("admin.memoryResourceCloud")}</Tag><Tag>{t("admin.memoryCloudReadOnly")}</Tag></Space>
    ...
    {loading ? <div className="cloud-resource-loading" role="status"><Spin /> {t("admin.memoryCloudLoading")}</div> : tree ? (
      <div className="cloud-resource-detail-body">
        <aside aria-label={t("admin.memoryCloudDirectory")}><Tree height={560} showLine blockNode defaultExpandAll selectedKeys={[selected]} treeData={nodes} onSelect={(keys, info) => { if (info.node.isLeaf && keys[0]) { setSelected(String(keys[0])); setTab("documents") } }} /></aside>
        <main>
          {resourceType === "workflow" ? <Tabs activeKey={tab} onChange={setTab} items={[
            { key: "graph", label: t("admin.memoryCloudWorkflowDefinition"), children: graphFiles["workflow.yaml"] && graphFiles["scenario/state.yml"] ? <div className="cloud-resource-graph"><StateGraphEditor key={tree.content_hash} readonly initialWorkflowYaml={graphFiles["workflow.yaml"]} initialStateYaml={withWorkflowLayout(graphFiles["scenario/state.yml"], graphFiles["scenario/layout.json"])} initialScenarioContent={graphFiles["scenario/scenario.md"]} workflowName={metadata?.resource_name} showEmptyHint={false} /></div> : <Alert type="info" message={t("admin.memoryCloudGraphUnavailable")} /> },
