...
vi.mock("./LegalLanguageToggle.scss", () => ({}));
...
describe("LegalLanguageToggle", () => {
  it("renders the current language as the selected value", () => {
    renderWithProviders(<LegalLanguageToggle />);
  ...
  it("changes the language and persists it to localStorage when a new option is chosen", () => {
    ...
    const option = screen.getAllByText("English").find(
      (el) => el.className.includes("ant-select-item-option-content"),
    );
